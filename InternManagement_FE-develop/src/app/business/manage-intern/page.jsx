                            ...
                            key !== 'cv_file' && key !== 'apply_id' && (
                                ...
                                >
                                    <span className={cx('sub-content', {
                                        ...
                                        'order': key === 'idx',
                                        'success-status': key === 'status' && student[key] === APPLY_STATUS.FINISHED.name,
                                        'fail-status': key === 'status' && student[key] === APPLY_STATUS.FAILED.name,
                                        'applying-status': key === 'status' && student[key] === APPLY_STATUS.APPLYING.name,
                                        'interviewing-status': key === 'status' && student[key] === APPLY_STATUS.INTERVIEWING.name,
                                        'onboard-status': key === 'status' && student[key] === APPLY_STATUS.ONBOARD.name,
                                    })}>{ student[key]} </span>
                                </div>
                            )
                        }
...

            {
                showOptions && (
                    <div className={cx('option-list')}>
                        <a href={student.cv_file} target="_blank" className={cx('sub-content')} style={{ fontWeight: 400, textDecoration: 'none' }}>
                            <FontAwesomeIcon icon={faEye} size="sm" style={{ marginRight: 8 }}/>
                            Xem CV
                        </a>
                        {
                            student.status === APPLY_STATUS.APPLYING.name && (
                                <React.Fragment>
                                    ...
                                    <span 
                                        ...
                                        style={{ fontWeight: 400 }}
                                    >
                                        <FontAwesomeIcon icon={faCalendarCheck} size="sm" style={{ marginRight: 8 }}/>
                                        Phỏng vấn
                                    </span>
                                </React.Fragment>
                            )
...
function InterningItem({ student, reload }) {
    ...
    const handleCompletedInternshipProgress = () => {
        if (!appreciationFile) {
            openNotificationWithIcon(
                    ...
                    <div className={cx('option-list')}>
                        ...
                        </span>
                        {
                            student.status === APPLY_STATUS.ONBOARD.name && (
                                ...
                                >
                                    <FontAwesomeIcon icon={faAirbnb} size="sm" style={{ marginRight: 8 }}/>
                                    Theo dõi
                                </Link>
                            )
                        }
                                ...
                                appreciationFile?.name && (
                                    <React.Fragment>
                                        <button 
                                            ...
                                            onClick={() => setAppreciationFile(null)}
                                        >
                                            <FontAwesomeIcon 
                                                icon={faXmark} 
                                                size="lg" 
...
export default function BusinessIntern() {
    ...
    const handleSubmitLinkedSchool = (status, linkId) => {
        GeneralService
            .updateLinkedStatus(status, linkId)
            .then(() => {
                openNotificationWithIcon(
                    NOTIFICATION_TYPE.SUCCESS,
                    "Thông báo",
        ...
        <div className={cx('wrapper')}>
            ...
            <div className={cx('intern-container')}>
                <div className={cx('intern-main')}>
                    <h4 className={cx('category-heading')}>{label.business["request-list"]}</h4>
                    <div className={cx('mt-5')}>
                        {
                            loading ? <div className={cx('mt-4')}>
                                ...
                                applyList.length > 0 ? applyList.map((student, index) => (
                                    <RequestItem student={student} key={index} reload={getInternManagementList}/>
            ...
            <div className={cx('intern-container')}>
                <div className={cx('intern-main')}>
                    <h4 className={cx('category-heading')}>{label.business["intern-list"]}</h4>
                    <div className={cx('mt-5')}>
                        {
                            loading ? <div className={cx('mt-4')}>
                                <Skeleton.Button 
            ...
            <div className={cx('intern-container')}>
                <div className={cx('intern-main')}>
                    <h4 className={cx('category-heading')}>{label.business["linked-school"]}</h4>
                    <div className={cx('mt-4')}>
                        {
                            loading ? <div className={cx('mt-4')}>
                                <Skeleton.Button 
                                    ...
                                    <div className={cx('row mb-3')} key={index}>
                                        <div className={cx(
                                            ...
                                            <div className={cx('d-flex gap-4')}>
                                                ...
                                                <div className={cx('ms-2 d-flex flex-column justify-content-center')}>
                                                    <span 
                                                        className={cx('sub-content', 'mb-1', 'semibold')}
                                                    >{item.school.school_name}</span>
                                                    <span 
                                                        className={cx('sub-content')}
                                                    >{item.school.shorthand_name}</span>
                                                </div>
                                            </div>
                                        </div>

                                        <div 
                                            className={cx('col-xl-4 col-lg-6 col-md-4 col-sm-2 align-items-center')}
                                        >
                                            <div 
                                        ...
                                        <div className={cx(
                                            'col-xl-4 col-lg-3 col-md-4 col-sm-4'
                                        )}>
                                                ...
                                                </button>
                                                {
                                                    item.is_linked === LINKED_STATUS.WAITING.value && (
                                                        <React.Fragment>
                                                            <button 
                                                                className={cx('view-btn')}
                                                                onClick={() => handleSubmitLinkedSchool(LINKED_STATUS.APPROVED.value, item.id)}
                                                            >
                                                                <FontAwesomeIcon 
                                                                    icon={faCheck} 
                                                                    size="sm" 
                                                            ...
                                                            <button 
                                                                className={cx('view-btn', 'error')}
                                                                onClick={() => handleSubmitLinkedSchool(LINKED_STATUS.REJECTED.value, item.id)}
                                                            >
                                                                <FontAwesomeIcon 
                                                                    icon={faXmark} 
                                                                    size="sm" 
