...
import { getRegistStatus, getPassedStatus, REGIST_STATUS, COVER_LETTER_STATUS, PASSED_STATUS } from "@/constant/intern-status";
...
function SubjectContainer({ subjects, setSubject, setViewHistory, setViewCoverLetter, registedSubject }) {
    ...
    return (
        <React.Fragment>
            ...
            <Row className={cx("intern-main")}>
                {
                    ...
                    subjects.map((subject, index) => (
                        <div className={cx('col-xl-3 col-lg-4 col-md-6 col-sm-12 col-12')} key={index}>
                            <div className={cx("subject-item")}  onClick={() => setSubject(subject)}>
                                <h4 className={cx("subject-field", "order", {
                                ...
                                <h4 className={cx("subject-field")}>
                                    ...
                                    />
                                    <span>
                                        { subject.max_students - (subject.availableSlots || 0) } / { subject.max_students }
                                    </span>
                                </h4>
                            </div>
                        </div>
            ...
            <div className={cx('d-flex gap-2 flex-wrap mt-2')} style={{ marginLeft: 12 }}>
                <button className={cx("view-btn")} onClick={handleOpenViewHistory}>
                    <FontAwesomeIcon icon={faClock} style={{marginRight: 6}}/>
                    Xem lịch sử đăng ký
                </button>
                <button className={cx("view-btn", "optional")} onClick={() => setViewCoverLetter(true)}>
            ...
            <Row style={{marginTop: 30, marginLeft: 6, marginRight: 6}}>
                <Col xs={12} md={12} lg={12} xl={12}>
                    <div className={cx("upload-container")}>
                        ...
                        <span className={cx("upload-title")}>File đánh giá từ Doanh nghiệp</span>
                        <input 
                            type="file" 
                            id="business-file" 
                            onChange={(e) => setBusinessFile(e.target.files[0])}
                        />
                        <label htmlFor="business-file" className={cx("upload-label")}>Tải lên</label>
                    </div>
                    <div className={cx("d-flex flex-column mt-2")}>
                        ...
                        <div className={cx("d-flex")}>
                            ...
                            />
                            {
                                businessFile?.name && (
                                    <React.Fragment>
                                        <button className={cx("view-btn", "btn-upload-option", "read")}>
                                            <FontAwesomeIcon 
                                        ...
                                        <button className={cx("view-btn", "btn-upload-option", "download")}>
                                            <FontAwesomeIcon 
                                                className={cx("btn-option__icon")} 
                                                icon={faDownload} 
                                                size="lg" 
                                        ...
                                        <button className={cx("view-btn", "btn-upload-option", "update")}>
                                            <FontAwesomeIcon 
                                                className={cx("btn-option__icon")} 
                                                icon={faPen} 
                                                size="lg" 
                                        ...
                                        <button className={cx("view-btn", "btn-upload-option", "delete")}>
                                            <FontAwesomeIcon 
                                                className={cx("btn-option__icon")} 
                                                icon={faXmark} 
                                                size="lg" 
                ...
                <Col xs={12} md={12} lg={12} xl={12}>
                    <div className={cx("upload-container")}>
                        ...
                        />
                        <label htmlFor="report-file" className={cx("upload-label")}>Tải lên</label>
                    </div>
                    <div className={cx("d-flex flex-column mt-2")}>
                        <label 
                            className={cx("upload-link__label")} 
        ...
        <Container className={cx('mt-4')}>
            <Row>
                ...
                <Col xl={3} lg={3} md={4} xs={6} className={cx('mb-3')}>
                    ...
                    <h5 className={cx("subject-content")}>
                        { 
                            formattedDate(isCancelable ? 
                                subject.internSubject.createdAt : 
                                subject.createdAt) 
                        }
                    </h5>
                </Col>
                ...
                <Col xl={3} lg={3} md={4} xs={6} className={cx('mb-3')}>
                    ...
                    <h5 className={cx("subject-content")}>
                        { 
                            isCancelable ? 
                                subject.internSubject.sessions : 
                                subject.sessions 
                        }
                    </h5>
                </Col>
                    ...
                    isCancelable && (
                        <React.Fragment>
                            <Col xl={3} lg={3} md={4} xs={6} className={cx('mb-3')}>
                                ...
                                <h5 className={cx("subject-content")}>{ formattedDate(subject.createdAt) }</h5>
                            </Col>
                            <Col xl={3} lg={3} md={4} xs={6} className={cx('mb-3')}>
                                ...
                                >{ getPassedStatus(subject.passed_status) }</span>
                            </Col>
                            <Col xl={3} lg={3} md={4} xs={6} className={cx('mb-3')}>
                                <p className={cx("subject-label")}>Trạng thái xác nhận đăng ký</p>
                                <h5 className={cx("subject-content")}>{ getRegistStatus(subject.regist_status) }</h5>
                            </Col>
                        </React.Fragment>
                    )
                }
...
export default function StudentIntern() {
    const [currentStatus, setCurrentStatus] = useState(INTERN_STATUS["REGIST-SUBJECT"]);
    ...
    function getOpeningSubjects() {
        ...
        InternService
            .getOpeningInternSubjects(schoolId)
            .then((res) => {
    ...
    const onRegistSubject = () => {
        InternService
            .registInternSubject(selectedSubject.id)
            .then(() => {
                openNotificationWithIcon(
                    NOTIFICATION_TYPE.SUCCESS,
                    'Thành công',
    ...
    const handleCancelCoverLetterRequest = () => {
        if (coverLetterRequest) {
            ...
            InternService
                .deleteCoverLetterRequest({ userId, requestId })
                .then(() => {
                    openNotificationWithIcon(
                        NOTIFICATION_TYPE.SUCCESS,
                        'Thành công',
    ...
    return (
        <div className={cx('wrapper')}>
                    ...
                    <div className={cx('intern-container', 'step-container-mobile')}>
                        {
                            steps.length > 0 && steps.map((step, index) => (
                                ...
                                >
                                    ...
                                    >
                                        <FontAwesomeIcon className={cx('step-icon')} icon={step.icon} size="xs"/>
                                    </div>
                                </Tooltip>
                            ))
                        }
                    ...
                    <div className={cx('intern-container', 'content-wrapper')}>
                        {
                            ...
                            (
                                isLoading ? (
                                    <div className={cx('d-flex align-items-center justify-content-center h-100')}>
                                        <Spin size="small"/>
                                    </div>
                                ) : (
                                    <SubjectContainer 
                        ...
                        {
                            currentStatus === INTERN_STATUS["REPORT"] &&
                            <ReportContainer />
                        }
                        {
            ...
            <Modal
                ...
                footer={[
                    <div className={cx('d-flex gap-1 justify-content-end')}>
                        <React.Fragment>
                            {
                                coverLetterRequest?.regist_submit_status === COVER_LETTER_STATUS.WAITTING.value && (
                                    <Popconfirm
                                        title="Hủy yêu cầu"
            ...
            >
                <React.Fragment>
                    {
                        coverLetterRequest && coverLetterRequest.regist_submit_status === COVER_LETTER_STATUS.SENT.value && (
                            <React.Fragment>
