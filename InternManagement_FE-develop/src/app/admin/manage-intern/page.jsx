...
import { Empty, Input, Modal, Select, Skeleton, Spin, Table, Tabs, notification } from "antd";
...
import { COVER_LETTER_STATUS, REGIST_STATUS, getCoverLetterStatus, getRegistStatus } from "@/constant/intern-status";
...
const RequestTabView = ({ data, reload, openNotificationWithIcon, isLoading }) => {
    ...
    const [selectedCoverLetterRequestId, setSelectedCoverLetterRequestId] = useState(null);
    ...
    const handleUpdateRegistSubjectStatus = (status, learnId) => {
        ...
        InternService
            .updateRegistSubjectRequest(data)
            .then(() => {
    ...
    const handleSubmitCoverLetterRequest = () => {
        ...
        GoogleService
            .uploadFile(coverLetterFile, UPLOAD_FILE_TYPE.DOCS)
            .then((coverLink) => {
                InternService
                    .submitCoverLetterRequest({ requestId: selectedCoverLetterRequestId, file: coverLink })
                    .then(() => {
                        openNotificationWithIcon(
                            NOTIFICATION_TYPE.SUCCESS,
                            "Thành công",
    ...
    return (
        <React.Fragment>
            <div>
                ...
                {
                    isLoading ? <div className={cx('mt-4')}>
                        <Skeleton.Button 
                            className={cx('mb-2')} 
                            active={true} 
                            size={"large"} 
                                                ...
                                                Object.keys(subjectRequestColumns).map((key, idx) => (
                                                    <div 
                                                        className={cx({
                                                            ...
                                                            'col-xl-2 col-lg-2 col-md-2 col-sm-2 col-2': key !== 'idx',
                                                        })} 
                                                        key={idx}
                                                    >
                                                        <span className={cx('sub-content', 'semibold', {
                                                        })}>{ subjectRequestColumns[key] } </span>
                                                    </div>
                                                ))
                                            }
                                        </div>
                                        {
                                            subjectRequests.map((request, index) => (
                                                ...
                                                >
                                                            ...
                                                            >
                                                                <span 
                                                                    className={cx('sub-content', {
                                                                        'order': key === 'idx',
                                                                        'success-status': key === 'regist_status' && request[key] === REGIST_STATUS.SUCCESSED.name,
                                                                        'fail-status': key === 'regist_status' && request[key] === REGIST_STATUS.REJECTED.name,
                                                                        'pending-status': key === 'regist_status' && request[key] === REGIST_STATUS.REGISTERING.name
                                                                    })}
                                                                >{ request[key] } </span>
                                                            </div>
                                                        ))
                                                    ...
                                                    {
                                                        subjectRequestList?.items[index].regist_status === REGIST_STATUS.REGISTERING.value && (
                                                            <div 
                                                                className={cx('col-xl-3 col-lg-3 col-md-3 col-sm-3 col-3', 'd-flex gap-2')}
                                                            >
                                                                <button 
                                                                    className={cx('view-btn', 'optional')} 
                                                                    onClick={() => handleUpdateRegistSubjectStatus(
                                        ...
                                        </div>
                                        {
                                            internRequests.map((request, index) => (
                                                ...
                                                >
                                                        ...
                                                        Object.keys(request).map((key, _index) => (
                                                            ...
                                                            >
                                                                <span 
                                                                    className={cx('sub-content', {
                                                                        'order': key === 'idx',
                                                                        'success-status': key === 'regist_submit_status' && request[key] === COVER_LETTER_STATUS.SENT.name,
                                                                        'fail-status': key === 'regist_submit_status' && request[key] === COVER_LETTER_STATUS.REJECTED.name,
                                                                        'pending-status': key === 'regist_submit_status' && request[key] === COVER_LETTER_STATUS.WAITTING.name
                                                                    })}
                                                                >{ request[key] } </span>
                                                            </div>
                                                        ))
                                                    ...
                                                    {
                                                        internRequestList?.items[index].regist_submit_status === COVER_LETTER_STATUS.WAITTING.value && (
                                                            <div 
                                                                className={cx('col-xl-3 col-lg-3 col-md-3 col-sm-3 col-3', 'd-flex gap-2')}
                                                            >
                                                                <button 
                                                                    ...
                                                                    onClick={() => {
                                                                        ...
                                                                        setSelectedCoverLetterRequestId(internRequestList?.items[index].id);
                                                                    }}
                                                                >
                                                                    { isPendingRequest && <Spin size="small" style={{ marginRight: 8 }}/> }
                                                                    Xác nhận
                                                                </button>
                                                                <button className={cx('view-btn', 'error')}>Hủy bỏ</button>
                                                            </div>
            ...
            <Modal
                ...
                footer={[
                    <div className={cx('d-flex justify-content-end gap-1')}>
                        <button className={cx('view-btn')} onClick={() => handleSubmitCoverLetterRequest()}>Xác nhận</button>
                        <button className={cx('view-btn', 'error')}>Hủy bỏ</button>
                    </div>
                ]}
            >
                <CoverLetterContainer coverLetterFile={coverLetterFile} setCoverLetterFile={setCoverLetterFile}/>
            </Modal>
        </React.Fragment>
    )
}
...
const OpenSubject = ({ schoolId, setShow, setSubject }) => {
    ...
    const getOpeningInternSubjects = () => {
        InternService
            .getOpeningInternSubjects(schoolId, curentAcademicYear, currentSemester)
            .then((res) => {
                                    ...
                                    subjects.map((subject, index) => (
                                        <div className={cx('row align-items-center m-2')} key={index}>
                                            ...
                                            <div className={cx('col')}>
                                                <span className={cx('item-content')} style={{ fontWeight: 600 }}>{subject.name}</span>
                                            </div>
                                            <div className={cx('col')}>
                                                <span className={cx('item-content')}>{formattedDate(subject.createdAt)}</span>
                                            </div>
                                            <div style={{ width: 50, cursor: 'pointer' }} onClick={() => {
                                                setSubject(subject);
                                                setShow();
                                            }}>
                                                <FontAwesomeIcon icon={faPenToSquare} size="sm" color="var(--red-5-color)" />
                                            </div>
                                        </div>
                                    ))
                                )
...
export default function AdminIntern() {
    ...
    const getRegistSubjectRequestList = (schoolId) => {
        InternService
            .getRegistSubjectRequestList(schoolId)
            .then((res) => setSubjectRequestList(res.data))
    }
    ...
    const postSubject = () => {
        ...
        InternService
            .createSubject(school.school_id, selectedDepartment, subject)
            .then(() => {
                openNotificationWithIcon(
                    NOTIFICATION_TYPE.SUCCESS,
                    "Thành công",
    ...
    return (
        <div className={cx("wrapper")}>
            ...
            <div className={cx('row')}>
                <div className={cx('col-xl-8 col-lg-7 col-md-12 col-sm-12 col-12', 'main-view')}>
